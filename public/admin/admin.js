// ===== Admin Dashboard Module (Express API) =====
...
async function openUploadWallpaperModal() {
  ...
  // Show/hide color input based on category selection
  ...
  if (categorySelect && colorGroup) {
    const updateColorVisibility = () => {
      colorGroup.style.display = (categorySelect.value === '纯色' || categorySelect.value === 'solid') ? 'block' : 'none';
    };
